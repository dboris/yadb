...
QUnit.test('read int32',function(){
    var topic=new Yadb3_fs('i32.ydb');
		var out=topic.readUI32(topic.signature_size);
		deepEqual(out,16843009,'uint 32 '+out);
		topic.free();
})
QUnit.test('read int8',function(){
		var topic=new Yadb3_fs('i8.ydb');
		var out=topic.readUI8(topic.signature_size);
		deepEqual(out,170,'uint 8 '+out);
		topic.free();
});
QUnit.test('read utf8',function(){
		var topic=new Yadb3_fs('utf8.ydb');
		var out=topic.readString(topic.signature_size,3);
		deepEqual(out,'abc','utf8 string '+out);
		topic.free();
});
QUnit.test('read ucs2',function(){
		var topic=new Yadb3_fs('ucs2.ydb');
		var out=topic.readString(topic.signature_size,4,'ucs2');
		deepEqual(out,'一丁','ucs2 string');
		topic.free();
});

QUnit.test('read pint',function(){
		var topic=new Yadb3('pint.ydb');
		var out=topic.load();
		deepEqual(out,[128,129],'pint'+out);
		topic.free();
});
QUnit.test('read vint',function(){
		var topic=new Yadb3('vint.ydb');
		var out=topic.load();
		deepEqual(out,[128,1],'vint'+out);
		topic.free();
});

QUnit.test('utf8 string array',function(){
		var topic=new Yadb3('arr_utf8.ydb');
		var out=topic.load();
		deepEqual(out,['abc','xyz'],'utf8 array'+JSON.stringify(out));
		topic.free();
});
QUnit.test('ucs2 string array',function(){
		var topic=new Yadb3('arr_ucs2.ydb');
		var out=topic.load();
